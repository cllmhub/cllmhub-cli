...
type Bridge struct {
	model       string
	backendType string // "ollama", "vllm", "lmstudio", "mlx", "llamacpp"
...
// StartBridge creates and starts a bridge for a model served by an external backend.
func (bm *BridgeManager) StartBridge(spec PublishModelSpec, hubURL, token string, tokenMgr *auth.TokenManager) error {
