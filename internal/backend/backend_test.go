...
func TestLlamaCpp_ConcurrentSlots(t *testing.T) {
	...
	b, _ := NewLlamaCpp(Config{URL: srv.URL})
	n, err := b.ConcurrentSlots(context.Background())
	if err != nil {
		t.Fatalf("ConcurrentSlots: %v", err)
	}
	if n != 3 {
		t.Errorf("ConcurrentSlots = %d, want 3", n)
	}
}

func TestLlamaCpp_ConcurrentSlots_Unavailable(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusNotFound)
	}))
	defer srv.Close()
	...
	b, _ := NewLlamaCpp(Config{URL: srv.URL})
	_, err := b.ConcurrentSlots(context.Background())
	if err == nil {
		t.Fatal("expected error when /slots returns 404")
	}
}

func TestProbeConcurrentSlots_Parallel(t *testing.T) {
	// Simulate a backend that can handle 4 concurrent requests.
	const maxParallel = 4
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// If more than maxParallel in-flight, delay significantly to simulate queuing.
		if int(n) > maxParallel {
	...
	b, _ := NewVLLM(Config{URL: srv.URL, Model: "test"})
	n, err := b.ConcurrentSlots(context.Background())
	if err != nil {
		t.Fatalf("ConcurrentSlots: %v", err)
	}
...
func TestProbeConcurrentSlots_SingleOnly(t *testing.T) {
	// Simulate a backend that serializes everything.
	var inflight atomic.Int32
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Any concurrency causes a big delay.
		if int(n) > 1 {
	...
	if n != 1 {
		t.Errorf("ConcurrentSlots = %d, want 1", n)
	}
}

func TestProbeConcurrentSlots_BackendDown(t *testing.T) {
	b, _ := NewVLLM(Config{URL: "http://127.0.0.1:1", Model: "test"})
	n, err := b.ConcurrentSlots(context.Background())
	if err != nil {
		t.Fatalf("ConcurrentSlots: unexpected error %v", err)
	}
	if n != 0 {
		t.Errorf("ConcurrentSlots = %d, want 0 for unreachable backend", n)
	}
}
